import type {ComponentVDOMNode, ElementVDOMNode, PortalVDOMNode, VDOMNode} from './types';
...
export function areNodesEqual(nodeOne: VDOMNode, nodeTwo: VDOMNode): boolean {
  ...
  if (nodeOne.type === DOM_TYPES.PORTAL) {
    const portalOne = nodeOne as PortalVDOMNode;
    const portalTwo = nodeTwo as PortalVDOMNode;
    return portalOne.container === portalTwo.container;
  }
  return true;
}
